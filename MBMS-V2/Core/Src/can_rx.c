#include "can_rx.h"
#include "main.h"
#include "cmsis_os2.h"

extern FDCAN_HandleTypeDef hfdcan1;
extern osMessageQueueId_t canRxQueueHandle;

volatile uint32_t g_rx_cb_hits = 0;

static void CAN_Rx(void);

void CAN_Rx_Task(void *argument)
{
    (void)argument;

    for (;;)
    {
        CAN_Rx();
    }
}

static void CAN_Rx(void)
{
    CANmsg msg;

    osStatus_t status = osMessageQueueGet(canRxQueueHandle, &msg, NULL, osWaitForever);

    if (status == osOK)
    {
        if (msg.extendedID == 0x100 &&
            msg.DLC == FDCAN_DLC_BYTES_8 &&
            msg.data[0] == 0x11 &&
            msg.data[1] == 0x22 &&
            msg.data[2] == 0x33 &&
            msg.data[3] == 0x44 &&
            msg.data[4] == 0x55 &&
            msg.data[5] == 0x66 &&
            msg.data[6] == 0x77 &&
            msg.data[7] == 0x88)
        {
            HAL_GPIO_TogglePin(RED_LED_GPIO_Port, RED_LED_Pin);
        }
    }
}

void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) == 0U)
        return;

    g_rx_cb_hits++;

    CANmsg msg = {0};
    FDCAN_RxHeaderTypeDef rxHeader;

    if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &rxHeader, msg.data) != HAL_OK)
        return;

    msg.extendedID = rxHeader.Identifier;
    msg.ID         = (uint16_t)(rxHeader.Identifier & 0x7FF);
    msg.DLC        = rxHeader.DataLength;

    (void)osMessageQueuePut(canRxQueueHandle, &msg, 0, 0);
}
